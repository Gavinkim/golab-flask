...
from api.model import UserRepository, TweetRepository
from api.service import UserService, TweetService
...
def user_service():
    ...
    return UserService(UserRepository(database), config.test_config, mock_s3_client)


@pytest.fixture
def tweet_service():
    return TweetService(TweetRepository(database))
